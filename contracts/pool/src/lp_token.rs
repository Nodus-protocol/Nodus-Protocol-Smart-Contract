...
pub fn mint(env: &Env, to: &Address, amount: i128) -> Result<(), Error> {
    ...
    env.storage()
        .instance()
        .set(&DataKey::LpTotalSupply, &new_supply);
    Ok(())
}
...
pub fn burn(env: &Env, from: &Address, amount: i128) -> Result<(), Error> {
    ...
    env.storage()
        .instance()
        .set(&DataKey::LpTotalSupply, &(supply - amount));
    Ok(())
}
...
pub fn transfer(env: &Env, from: &Address, to: &Address, amount: i128) -> Result<(), Error> {
    ...
    env.storage()
        .persistent()
        .extend_ttl(&from_key, TTL_THRESHOLD, TTL_BUMP);
    env.storage().persistent().set(&to_key, &to_new);
    env.storage()
        .persistent()
        .extend_ttl(&to_key, TTL_THRESHOLD, TTL_BUMP);
    Ok(())
}
